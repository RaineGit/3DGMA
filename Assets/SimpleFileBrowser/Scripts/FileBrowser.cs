...
#pragma warning disable 0649
		...
		[SerializeField]
		private LeanToggle showHiddenFilesToggle;
...
#pragma warning restore 0649
		...
		void Start(){
			SetFilters(true, new string[] {".png", ".jpg", ".jpeg", ".gif"});
		}
		...
		public void OnSubmitButtonClicked()
		{
			...
			if( filenameInput.Length > 0 )
				...
				//FileUtil.CopyFileOrDirectory(path, Application.persistentDataPath+"/image.png");
				GameObject.FindWithTag("Controller").GetComponent<Controller>().test(path);
			if( File.Exists( path ) )
			{
				if( !m_folderSelectMode )
					OnOperationSuccessful( path );
