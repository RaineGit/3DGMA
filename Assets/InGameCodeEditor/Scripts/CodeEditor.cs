...
namespace InGameCodeEditor
{
    ...
    public class CodeEditor : MonoBehaviour
    {
        ...
        int lastAmountOfLinesShown = 0;
        RectTransform caret;
        float inputTextStartXPos = 0f;
        List<string> textAsArray = new List<string>(){""};
...
#pragma warning restore 0649
        ...
        public void Start()
        {
            ...
            ApplyTheme();
            ApplyLanguage();
            GetComponent<RectTransform>().localScale = new Vector2(Screen.width/480f, Screen.width/480f);
            GetComponent<RectTransform>().sizeDelta = new Vector2(GetComponent<RectTransform>().sizeDelta.x, GetComponent<RectTransform>().sizeDelta.y * (((float)Screen.height/(float)Screen.width*480f)/853.333f));
            inputTextStartXPos = inputText.rectTransform.anchoredPosition.x;
            firstLineShown = 0;
            lastAmountOfLinesShown = (inputField.text.Split('\n').Length > 10 ? 10 : inputField.text.Split('\n').Length);
            RemoveStrangeCharacter();
            UpdateHorizontalScrollbar();
        }

        public void Update(){
            lineText.rectTransform.anchoredPosition = new Vector2(lineText.rectTransform.anchoredPosition.x, inputText.rectTransform.anchoredPosition.y);
            inputText.rectTransform.anchoredPosition = new Vector2(inputField.textComponent.rectTransform.anchoredPosition.x, inputText.rectTransform.anchoredPosition.y);
        }

        public void UpdateHorizontalScrollbar(){
            Vector2 renderedValues = inputText.GetRenderedValues(false);
            float newSize = inputField.textViewport.rect.width / renderedValues.x * 0.9f;
            horizontalScrollbar.size = (newSize < 0.1f ? 0.1f : newSize);
            horizontalScrollbar.value = (-inputField.textComponent.rectTransform.anchoredPosition.x) / (renderedValues.x - inputField.textViewport.rect.width);
            if(horizontalScrollbar.size >= 1){
                inputText.rectTransform.anchoredPosition = new Vector2(inputTextStartXPos, inputText.rectTransform.anchoredPosition.y);
            }
        }

        public void UpdateHorizontalPosition(){
            Vector2 renderedValues = inputText.GetRenderedValues(false);
            inputField.textComponent.rectTransform.anchoredPosition = new Vector2(-horizontalScrollbar.value * (renderedValues.x - inputField.textViewport.rect.width), inputField.textComponent.rectTransform.anchoredPosition.y);
            if(inputField.textComponent.rectTransform.anchoredPosition.x > inputTextStartXPos){
                inputField.textComponent.rectTransform.anchoredPosition = new Vector2(inputTextStartXPos, inputField.textComponent.rectTransform.anchoredPosition.y);
            }
            if(caret == null && gameObject.active){
                foreach(Transform child in inputText.gameObject.transform.parent){
                    if(child.gameObject.name == "InputField Input Caret"){
                        caret = child.gameObject.GetComponent<RectTransform>();
                    }
                }
            }
            caret.anchoredPosition = new Vector2(inputText.rectTransform.anchoredPosition.x, caret.anchoredPosition.y);
        }

        public void UpdateVerticalPosition(){
            firstLineShown = (int)Math.Floor(verticalScrollbar.value*10);
            UpdateView();
        }

        public void RemoveStrangeCharacter(){
            inputField.text = inputField.text.Replace(evilCharacter.text, "");
            SaveTextToTemp();
            UpdateView();
            DisplayedContentChanged(inputField.text, false, false);
        }

        public void SaveTextToTemp(){
            for(int i=0; i<inputField.text.Split('\n').Length; i++){
                if(inputField.text.Split('\n').Length-1 >= i+firstLineShown){
                    int needed = (i + firstLineShown - textAsArray.Count()+1);
                    for(int j=0; j<needed; j++){
                        textAsArray.Add("");
                    }
                    string newVal = "";
                    try{
                        newVal = inputField.text.Split('\n')[i];
                    }
                    catch{}
                    textAsArray[i + firstLineShown] = newVal;
                }
            }
            if(inputField.text.Split('\n').Length > shownLines){
                firstLineShown += inputField.text.Split('\n').Length - shownLines;
            }
        }
        ...
        public void UpdateView(){
            ...
            for(int i=0; i<shownLines; i++){
                if(textAsArray.Count()-1 >= i+firstLineShown){
                    result.Add(textAsArray[i + firstLineShown]);
                }
                else{
            ...
            lastAmountOfLinesShown = result.Count();
            //Debug.Log(result);
        }
        ...
        private void UpdateCurrentLineNumbers()
        {
            ...
            if (currentLineCount != lineCount)
            {
                ...
                int realLine = 1;
                int prevRealLineIndex = 0;
                for (int i = 1; i < currentLineCount + 1; i++)
                {
                    lineBuilder.Append(prevRealLineIndex == realLine ? " " : realLine.ToString());
                    lineBuilder.Append('\n');   
                    prevRealLineIndex = realLine;
                    if(inputText.text.Substring(inputText.textInfo.lineInfo[i-1].lastCharacterIndex, 1)[0] == '\n'){
                        realLine++;
                    }         
                }
