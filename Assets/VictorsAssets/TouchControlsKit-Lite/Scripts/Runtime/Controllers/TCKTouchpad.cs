...
namespace TouchControlsKit
{
        ...
        IPointerUpHandler, IPointerDownHandler, IDragHandler, IPointerEnterHandler
    {
        ...
        void Start()
        {
            gizmoScript = GameObject.Find("player").transform.GetChild(0).GetComponent<TransformGizmo>();
        }
        ...
        {
            gizmoScript.wasCameraMoved = true;
            if( Input.touchCount >= touchId && touchDown && !gizmoScript.isTransforming )
            {
                UpdatePosition( pointerData.position );
