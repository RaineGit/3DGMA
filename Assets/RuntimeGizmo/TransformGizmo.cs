...
namespace RuntimeGizmos
{
	...
	public class TransformGizmo : MonoBehaviour
	{
		...
		public bool wasCameraMoved = false;
		public bool selectingGizmo = false;
		...
		void Update()
		{
			...
			if(targetRootsOrdered.Count==1){
				lastSelection=targetRootsOrdered[0];
			}
		...
		{
			if(mainTargetRoot != null && controller.inSceneEditingMode)
			{
		...
		{
			if(nearAxis == Axis.None && !wasCameraMoved)
			{
				...
				if(Physics.Raycast(myCamera.ScreenPointToRay(Input.mousePosition), out hitInfo, Mathf.Infinity, selectionMask))
				{
					...
					{
						ClearAndAddTarget( target.GetComponent<InstantFollow>() == null ? target : target.GetComponent<InstantFollow>().target.transform );
					}
					*/
					...
					if(!controller.multipleSelection){
						...
						AddTarget( target.GetComponent<InstantFollow>() == null ? target : target.GetComponent<InstantFollow>().target.transform );
					}
		...
		public void AddTarget(Transform target, bool addCommand = true)
		{
			if(target != null)
			{
				...
				controller.selectedTransforms.Add(target);
				if(target.GetComponent<ObjectController>() != null){
					target.GetComponent<ObjectController>().selected = true;
					if(target.GetComponent<ObjectController>().element != null){
						//target.GetComponent<ObjectController>().element.GetComponent<UnityEngine.UI.Image>().color = new Color(0, 0.67f, 1, 0.18f);
						target.GetComponent<ObjectController>().element.SetCheckboxOn(true);
					}
				}
				...
				SetPivotPoint();
				controller.openInspectorButton.SetActive(targetRoots.Count() != 0 && controller.panels[3].parent == controller.hiddenPanelSlot);
			}
		}
		...
		public void RemoveTarget(Transform target, bool addCommand = true)
		{
			if(target != null)
			{
				controller.selectedTransforms.Remove(target);
				if(target.GetComponent<ObjectController>() != null){
					target.GetComponent<ObjectController>().selected = false;
					if(target.GetComponent<ObjectController>().element != null){
						//target.GetComponent<ObjectController>().element.GetComponent<UnityEngine.UI.Image>().color = new Color(1, 1, 1, 0.078f);
						target.GetComponent<ObjectController>().element.SetCheckboxOn(false);
					}
				}
		...
		public void ClearTargets(bool addCommand = true)
		{
			...
			foreach(Transform child in targetRootsOrdered.ToList()){
				RemoveTarget(child);
			}

			foreach(Transform child in controller.selectedTransforms.ToList()){
				controller.selectedTransforms.Remove(child);
				if(child.GetComponent<ObjectController>() != null){
					child.GetComponent<ObjectController>().selected = false;
					if(child.GetComponent<ObjectController>().element != null){
						//child.GetComponent<ObjectController>().element.GetComponent<UnityEngine.UI.Image>().color = new Color(1, 1, 1, 0.078f);
						child.GetComponent<ObjectController>().element.SetCheckboxOn(false);
					}
				}
			}
			...
			if(target != null)
			{
				...
				for(int i = 0; i < renderersBuffer.Count; i++)
				{
					...
					if(!highlightedRenderers.Contains(render))
					{
						...
						*/
						if(render.GetComponent<MeshRenderer>() != null){
							var outline = render.gameObject.AddComponent<Outline>();
							if(controller.multipleSelection){
								outline.color = 1;
							}
						}
						else if(render.GetComponent<SpriteRenderer>() != null){
							render.GetComponent<SpriteRenderer>().color = new Color(1, 0.75f, 0.38f);
							render.transform.parent.GetComponent<InstantFollow>().arrow.gameObject.SetActive(true);
						}
		...
		void GetTargetRenderers(Transform target, List<Renderer> renderers)
		{
			...
			if(target != null)
			{
				...
				List<Light> lights = target.GetComponentsInChildren<Light>().ToList();
				foreach(Light light in lights){
					try{
						renderers.Add(light.GetComponent<ObjectController>().lightSprite.GetComponent<InstantFollow>().sprite);
					}
					catch{}
				}
				List<Camera> cameras = target.GetComponentsInChildren<Camera>().ToList();
				foreach(var child in cameras){
					try{
						renderers.Add(child.GetComponent<ObjectController>().lightSprite.GetComponent<InstantFollow>().sprite);
					}
					catch{}
				}
			}
			...
			foreach(Transform target in targetRootsOrdered){
				if(target.GetComponent<MeshRenderer>()!=null){
					for(int i=0; i<target.GetComponent<MeshRenderer>().materials.Length; i++){
						if(target.GetComponent<MeshRenderer>().materials[i].name == "Custom/Outline (Instance)"){
							target.GetComponent<MeshRenderer>().materials = target.GetComponent<MeshRenderer>().materials.Where(w => w != target.GetComponent<MeshRenderer>().materials[i]).ToArray();
						}
					}
				}
			}
		...
		void RemoveHighlightedRenderers(List<Renderer> renderers)
		{
			for(int i = 0; i < renderersBuffer.Count; i++)
			{
				...
				if(render != null)
				{
					...
					*/
					if(render.GetComponent<MeshRenderer>() != null){
						Destroy(render.gameObject.GetComponent<Outline>());
					}
					else if(render.GetComponent<SpriteRenderer>() != null){
						render.GetComponent<SpriteRenderer>().color = new Color(1, 1, 1);
						render.transform.parent.GetComponent<InstantFollow>().arrow.gameObject.SetActive(false);
					}
				}
		...
		public void SetSpaceToGlobal(){
			space = TransformSpace.Global;
		}

		public void SetSpaceToLocal(){
